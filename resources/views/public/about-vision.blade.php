...
@section('title', ($brandAssets['brandTitle'] ?? 'Astha Academics') . ' | ' . $aboutPage['vision_title'])

@push('meta')
    <meta name="description" content="{{ str($aboutPage['vision_summary'])->limit(155) }}">
@endpush
    ...
    <div class="public-about-shell bg-[#f7f9fc]">
        <section class="mx-auto w-full max-w-[1080px] px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
            ...
            <article class="public-about-card mt-8 border border-slate-200 bg-white p-7 shadow-[0_22px_50px_rgba(15,23,42,0.06)] sm:p-9 lg:p-10">
                <p class="text-[13px] font-bold uppercase tracking-[0.18em] text-blue-600">{{ public_trans('public.about.vision_eyebrow') }}</p>
                <h1 class="home-title-font mt-4 text-[clamp(2rem,4vw,3.25rem)] font-black leading-tight text-slate-950">{{ $aboutPage['vision_title'] }}</h1>
                <p class="mt-5 max-w-[62ch] text-base leading-8 text-slate-600">{{ $aboutPage['vision_summary'] }}</p>
                <div class="mt-8 text-base leading-8 text-slate-600 sm:text-[1.05rem]">
                    @foreach (preg_split("/\r\n|\n|\r/", $aboutPage['vision_details']) as $paragraph)
