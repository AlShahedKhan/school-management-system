...
@section('title', 'Edit Dashboard News')
@section('page-title', 'Edit Dashboard News')
...
@section('content')
    <div class="min-h-screen bg-gray-50 p-4 md:p-6">
        <div class="mb-6 flex flex-col justify-between gap-4 border border-gray-100 bg-white p-4 shadow-sm md:flex-row md:items-center">
            <div>
                <h2 class="text-xl font-bold text-gray-800">Edit Dashboard News</h2>
                <p class="text-xs text-gray-500">Update the school dashboard news message.</p>
            </div>
        ...
        <form action="{{ route('admin.dashboard-news.update', $dashboardNews) }}" method="POST">
            @csrf
            @method('PUT')
            @php($submitLabel = 'Update News')
