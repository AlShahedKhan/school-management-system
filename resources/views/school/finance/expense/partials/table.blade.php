...
>
    ...
    <x-slot:head>
        ...
        <x-table.th unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-center font-semibold">Year</x-table.th>
        <x-table.th unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-center font-semibold">Month</x-table.th>
    ...
    @foreach ($expenses as $expense)
        <x-table.row unstyled class="hover:bg-gray-50">
            ...
            <x-table.td unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-center">
                {{ $expense->year ?? ($expense->date ? \Carbon\Carbon::parse($expense->date)->format('Y') : '-') }}
            </x-table.td>
            <x-table.td unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-center">
                {{ $expense->month ?? ($expense->date ? \Carbon\Carbon::parse($expense->date)->format('F') : '-') }}
            </x-table.td>
