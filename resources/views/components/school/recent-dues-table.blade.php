@props([
    'items' => [],
    'title' => 'Recent Dues',
    'titleId' => 'recent-dues-title',
    'emptyMessage' => 'No outstanding dues found.',
    'amountLabel' => 'Due',
    'dateLabel' => 'Due Date',
])
...
>
    <header class="mb-3 flex min-h-8 items-center">
        <h3 id="{{ $titleId }}" class="!m-0 truncate !text-[15px] !font-normal leading-tight text-gray-800 sm:!text-xl">
            {{ $title }}
        </h3>
    </header>
    ...
    >
        ...
        <x-slot:head>
            ...
            <x-table.th unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-right font-semibold">{{ $amountLabel }}</x-table.th>
            <x-table.th unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-left font-semibold">{{ $dateLabel }}</x-table.th>
        </x-slot:head>
        ...
        @foreach ($items as $item)
            <x-table.row unstyled class="hover:bg-gray-50">
                ...
                <x-table.td unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-right"><span class="text-xs font-semibold text-red-600">{{ $item['amount'] }}</span></x-table.td>
