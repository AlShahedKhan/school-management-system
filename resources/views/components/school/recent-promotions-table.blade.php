...
<section
    ...
    aria-labelledby="recent-promotions-title"
>
    <header class="mb-3 flex min-h-8 items-center">
        <h3 id="recent-promotions-title" class="!m-0 truncate !text-[15px] !font-normal leading-tight text-gray-800 sm:!text-xl">
            Recent Promotions
        </h3>
    </header>
    ...
    <x-school.data-table
        :empty="count($items) === 0"
        :empty-colspan="7"
        empty-message="No recent promotions found."
        min-width="900px"
    ...
    >
        ...
        <x-slot:head>
            ...
            <x-table.th unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-left font-semibold">From Class</x-table.th>
            <x-table.th unstyled class="h-8 whitespace-nowrap border border-gray-300 px-3 text-left font-semibold">To Class</x-table.th>
        ...
        @foreach ($items as $item)
            <x-table.row unstyled class="hover:bg-gray-50">
                ...
                <x-table.td unstyled class="h-8 border border-gray-300 px-3">
                    <x-school.table-cell-scroll :title="$item['from_class']">
                        <span class="text-xs text-slate-600">{{ $item['from_class'] }}</span>
                    </x-school.table-cell-scroll>
                </x-table.td>

                <x-table.td unstyled class="h-8 border border-gray-300 px-3">
                    <x-school.table-cell-scroll :title="$item['to_class']">
                        <span class="text-xs text-slate-600">{{ $item['to_class'] }}</span>
                    </x-school.table-cell-scroll>
                </x-table.td>
